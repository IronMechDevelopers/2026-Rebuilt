...
public class ButtonBindings {
  ...
  // private final CommandJoystick leftDriver;
  // private final CommandJoystick rightDriver;
  private final CommandXboxController driver;
  private final CommandPS4Controller coDriver;
   ...
   *
   * <p>
   * Call this once from RobotContainer after creating all subsystems.
   */
  ...
  private void configureDriverBindings() {
    driver.back().onTrue(DriveCommands.zeroHeading(driveSubsystem));
    ...
    driver.y().whileTrue(DriveCommands.snapToClosestCardinal(
        driveSubsystem,
        () -> -driver.getLeftY(),
        () -> -driver.getLeftX()));
    ...
    driver.x().whileTrue(DriveCommands.xStance(driveSubsystem));

    driver.povUp().toggleOnTrue(DriveCommands.setSpeed(driveSubsystem, 1.0));
    driver.povDown().toggleOnTrue(DriveCommands.setSpeed(driveSubsystem, .25));
    driver.povRight().toggleOnTrue(DriveCommands.setSpeed(driveSubsystem, .5));
    driver.povLeft().toggleOnTrue(DriveCommands.setSpeed(driveSubsystem, .75));
  ...
  private void configureCoDriverBindings() {
    ...
    coDriver.triangle().whileTrue(ballSubsystem.slowIntakeCommand().finallyDo(() -> ballSubsystem.stop()));
  }
}
